...
<!-- - 头脑风暴
  - 性能指标分类
    - 应用方面：工程师角度（响应时间、请求数、请求的错误率）
    - 用户体验方面：用户角度（首屏、白屏）
    - 服务器方面：服务器（CPU、内存、网络带宽、流量）
  - 性能指标
    - CPU
    - FPS
  - 关于指标解读
    ...
    - 如何分析、测量（查看的工具、方法）
    - 如何改善
  - 更进一步，性能监控平台？（GIS 已经有服务在弄了，这个咨询下平台、可以使用 web 性能优化提供的 api，做一个简版 demo） -->
...
左边红线代表的是网络传输层面的过程，右边红线代表了服务器传输回字节后浏览器的各种事件状态，这个阶段包含了浏览器对文档的解析、DOM 树构建、布局、绘制等。
